...
const setThrown = actionCreator<{ id: RubbishId; thrown: boolean }>(
  "THROW_RUBBISH"
);
...
export const initialState: IState = mapValues(rubbish, () => ({
  thrown: false
}));

export const reducer = reducerWithInitialState(initialState).case(
  setThrown,
  (state, { id, thrown }) => ({ ...state, [id]: { thrown } })
);
