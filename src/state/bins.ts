...
export const initialState: IState = mapValues(bins, () => ({ position: { x: 0, y: 0 }, shaken: false }));

export const reducer = reducerWithInitialState(initialState)
    .case(setPosition, (state, { id, position }) => ({ ...state, [id]: { ...state[id], position } }))
    .case(setShaken, (state, { id, shaken }) => ({ ...state, [id]: { ...state[id], shaken } }));
