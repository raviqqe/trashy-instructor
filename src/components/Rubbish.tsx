...
const Rubbish = styled.div<{ movement: IMovement; moving: boolean }>`
  ...
  ${({ movement, moving }) =>
    moving ? `animation: ${animation(movement)} ${duration}ms;` : ""} svg {
...
@connect(
  ({ bins, rubbish }, { id }: IProps) => ({ bins, ...rubbish[id] }),
  bins.actionCreators
)
export default class extends React.Component<IProps, IState> {
  ...
  public componentDidUpdate(props: IProps) {
    ...
    if (!moving && !props.thrown && thrown) {
      ...
      this.setState({
        movement: positionsToMovement(
          elementToPosition(this.ref.current),
          bins[binId].position
        ),
        moving: true
      });
